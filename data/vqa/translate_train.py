...
for lang, part in tqdm(mc4_lang_dist.items(), total=len(mc4_lang_dist)):
    ...
    if lang == "en":
        ...
        question = [ex["context"] for ex in examples]
        translates_question = translate(model, tokenizer, nllb_lang, question)
        for i, ex in enumerate(examples):
            ex["context"] = lang_code2lang[lang] + " # " + translates_question[i]
...
# Shuffle again so answering and generation are unlikely to share a language
np.random.shuffle(data)
mt_data = []
idx = 0
print(f"Total examples: {len(data)}")
for lang, part in tqdm(mc4_lang_dist.items(), total=len(mc4_lang_dist)):
    ...
    if lang == "en":
        ...
        for i, ex in enumerate(examples):
            ex["context"] = lang_code2lang[lang] + " # " + translates_question[i]
    mt_data.extend(examples)
with open(os.path.join(root, f"vqav2_mt_g_raw.json"), "w", encoding="utf-8") as f:
    json.dump(mt_data, f, ensure_ascii=False, indent=2)