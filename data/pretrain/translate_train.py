...
caption_templates = ["Caption the image.",
             "Short image caption: ",
             "Briefly describe the image.", "Write a short image description.",
             "Summarize the image."]

root = "."
file = "ccs_synthetic_filtered_large_2273005_en.json" # "ccs_synthetic_filtered_large_1000000_en.json"
...
templates_mt = dict(en=caption_templates)
for lang in tqdm(mc4_lang_dist, total=len(mc4_lang_dist)):
    if lang == "en":
        continue
    nllb_lang = mc42nllb[lang]
    translates_templates = translate(model, tokenizer, nllb_lang, caption_templates)
    translates_templates = [t for t in translates_templates if "image caption" not in t]
    templates_mt[lang] = translates_templates
with open(os.path.join(root, "caption_templates.json"), "w", encoding="utf-8") as f:
    json.dump(templates_mt, f, ensure_ascii=False, indent=2)
...
for lang, part in tqdm(mc4_lang_dist.items(), total=len(mc4_lang_dist)):
    ...
    if lang == "en":
        ...
        for i, ex in enumerate(examples):
            ex["context"] = lang_code2lang[lang]
            ex["label"] = translates_captions[i]
    mt_data.extend(examples)
with open(os.path.join(root, f"ccs_synthetic_filtered_large_2273005_mt_raw.json"), "w", encoding="utf-8") as f:
